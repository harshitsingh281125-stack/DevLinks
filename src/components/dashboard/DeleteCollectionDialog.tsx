      ...
      <div ref={panelRef} className="relative z-10 w-full max-w-md rounded-[2rem] border border-white/10 bg-ink-900 shadow-panel">
        <div className="flex items-start justify-between gap-4 rounded-t-[2rem] border-b border-white/10 bg-ink-900/95 p-6 backdrop-blur">
          <div className="flex items-center gap-3">
            ...
            <p className="text-base font-semibold text-white">Delete collection?</p>
          </div>
          <button
            type="button"
            onClick={onCancel}
        ...
        <div className="p-6 space-y-3">
          <p className="text-sm leading-6 text-sand-200/80">
            <span className="font-semibold text-white">&ldquo;{collection.name}&rdquo;</span> will
            be permanently deleted. This cannot be undone.
          </p>
          <p className="text-sm leading-6 text-sand-200/60">
            Deletion will fail if bookmarks still belong to this collection. Remove them first.
          </p>
        </div>
